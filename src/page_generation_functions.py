...
def generate_page(from_path, template_path):
    dest_file_path = from_path.replace("content", "public").replace(".md", ".html")
    ...
    final_html = title_template.replace("{{ Content }}", content_html_text)
    output_file = open(dest_file_path, "w")
    output_file.write(final_html)

def generate_page_recursive(from_path, template_path):
    dest_dir_path = from_path.replace("content", "public")
    ls = os.listdir(from_path)
    for item in ls:
        item_path = os.path.join(from_path, item)
        if os.path.isfile(item_path):
            generate_page(item_path, template_path)
        else:
            os.mkdir(os.path.join(dest_dir_path, item)) #if item is a directory, creates the directory in the public folder
            generate_page_recursive(item_path, template_path)
